...
const fields = {
  ...
  invoiceItems: 'Add at least one invoice item',
...
export const EditForm = forwardRef(function EditForm(
  {
    ...
    setToastMessage: React.Dispatch<React.SetStateAction<object>>
  ...
  const {
    client: { loading, statusCode, errorMessage },
  ...
  const onHandleSave = () => {
    ...
    !errors.length && dispatch(saveInvoice(selectedInvoice.uuid))
  }
        ...
        <Root defaultValue="invoice" orientation="vertical" className={styles.form_root}>
          <List className={styles.tab_list}>
            <Trigger className={styles.trigger} value="invoice" asChild>
              <motion.div>Invoice information</motion.div>
            </Trigger>
          ...
          <div className={styles.tab_content_cont}>
            <Content value="invoice" className={styles.tab_content}>
              <div>
                <Input
                  icon={<p style={{ fontSize: 12 }}>{settings?.prefix}-</p>}
                  type="text"
                  name="invoiceNumber"
                  ...
                  <div className={styles.halves}>
                    <div>
                      <DatePicker
                        ...
                        label="Due By"
                        selected={selectedInvoice?.dueDate as unknown as Date}
              ...
              <div className="input_group">
                <div className={styles.halves}>
                  {Object.entries(templateSettings?.theme || {})
                    .filter(([k, v]) => k === 'logo')
                    .map((i) => {
                      return templateSettings?.theme?.logo ? (
                        <div onClick={removeLogo} style={{ cursor: 'pointer' }}>
                          <span style={{ marginLeft: '6px', fontSize: 14 }}>Remove logo</span>
                        </div>
                      ) : (
                        <label
                          htmlFor="logo"
                        ...
                        >
                          ...
                          />
                          <Camera size={18} />
                          <span style={{ marginLeft: '6px', fontSize: 14 }}>Upload logo</span>
                        </label>
                      )
                    })}
                </div>
            ...
            <Content value="client" className={styles.tab_content}>
              <Select
                label={<p style={{ fontSize: 13 }}>Search Client</p>}
